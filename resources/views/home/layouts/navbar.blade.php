      ...
      <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
        ...
        <div class="ml-3 relative">
          ...
          <div class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none hidden" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
            ...
            <a href="#" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-0">个人资料</a>
            <a href="{{route('home.auth.logout')}}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="user-menu-item-2">退出登录</a>
          </div>
        </div>
        @endauth
        @guest
        <div class="ml-3 relative">
          <a class="bg-blue-500 text-white px-4 py-2 rounded-md" href="{{ route('home.auth.index') }}">用户登录</a>
        </div>
        @endguest
      </div>
    </div>
...
<script>
  ...
  function initNavbar(){
    ...
    if(navItem.length == 0) return ; //不显示分类
